...
func TestTemplateDirectorySessionOutcomeTransportRejectionFixture(t *testing.T) {
	fixturePath := filepath.Join(repoRoot(t), "fixtures", "diagnostics", "slice-408-template-directory-session-outcome-transport-rejection", "template-directory-session-outcome-envelope-rejection.json")
	fixture := readJSONFixture(t, fixturePath)
	fixtureRoot := filepath.Dir(fixturePath)

	for _, rawCase := range fixture["cases"].([]any) {
		testCase := rawCase.(map[string]any)
		envelope := decodeSessionOutcomeEnvelopeFromFixture(t, testCase["envelope"], fixtureRoot)
		expected := decodeSessionOutcomeTransportImportErrorFromFixture(testCase["expected_error"])

		imported, importErr := asttemplate.ImportSessionOutcomeEnvelope(envelope)
		if imported != nil {
			t.Fatalf("%s outcome rejection unexpectedly imported %+v", testCase["label"], imported)
		}
		if !reflect.DeepEqual(importErr, expected) {
			t.Fatalf("%s outcome rejection mismatch: got %+v want %+v", testCase["label"], importErr, expected)
		}
	}
}

func TestTemplateDirectorySessionOutcomeEnvelopeApplicationFixture(t *testing.T) {
	fixturePath := filepath.Join(repoRoot(t), "fixtures", "diagnostics", "slice-409-template-directory-session-outcome-envelope-application", "template-directory-session-outcome-envelope-application.json")
	fixture := readJSONFixture(t, fixturePath)
	fixtureRoot := filepath.Dir(fixturePath)
	...
	for _, rawCase := range fixture["rejections"].([]any) {
		...
		imported, importErr := asttemplate.ImportSessionOutcomeEnvelope(envelope)
		if imported != nil {
			t.Fatalf("%s outcome envelope rejection unexpectedly imported %+v", testCase["label"], imported)
		}
		if !reflect.DeepEqual(importErr, expected) {
			t.Fatalf("%s outcome envelope rejection mismatch: got %+v want %+v", testCase["label"], importErr, expected)
		}
	}
}
...
func TestTemplateDirectorySessionRequestTransportRejectionFixture(t *testing.T) {
	fixturePath := filepath.Join(repoRoot(t), "fixtures", "diagnostics", "slice-405-template-directory-session-request-transport-rejection", "template-directory-session-request-envelope-rejection.json")
	fixture := readJSONFixture(t, fixturePath)
	fixtureRoot := filepath.Dir(fixturePath)

	for _, rawCase := range fixture["cases"].([]any) {
		testCase := rawCase.(map[string]any)
		envelope := decodeSessionRequestEnvelopeFromFixture(t, testCase["envelope"], fixtureRoot)
		expected := decodeSessionRequestTransportImportErrorFromFixture(testCase["expected_error"])

		imported, importErr := asttemplate.ImportSessionRequestEnvelope(envelope)
		if imported != nil {
			t.Fatalf("%s request rejection unexpectedly imported %+v", testCase["label"], imported)
		}
		if !reflect.DeepEqual(importErr, expected) {
			t.Fatalf("%s request rejection mismatch: got %+v want %+v", testCase["label"], importErr, expected)
		}
	}
}

func TestTemplateDirectorySessionRequestEnvelopeApplicationFixture(t *testing.T) {
	fixturePath := filepath.Join(repoRoot(t), "fixtures", "diagnostics", "slice-406-template-directory-session-request-envelope-application", "template-directory-session-request-envelope-application.json")
	fixture := readJSONFixture(t, fixturePath)
	fixtureRoot := filepath.Dir(fixturePath)
	...
	for _, rawCase := range fixture["cases"].([]any) {
		...
		actual, err := asttemplate.RunTemplateDirectorySessionRequest(*imported)
		if err != nil {
			t.Fatalf("%s request envelope application failed: %v", testCase["label"], err)
		}
	...
	for _, rawCase := range fixture["rejections"].([]any) {
		...
		imported, importErr := asttemplate.ImportSessionRequestEnvelope(envelope)
		if imported != nil {
			t.Fatalf("%s request envelope rejection unexpectedly imported %+v", testCase["label"], imported)
		}
		if !reflect.DeepEqual(importErr, expected) {
			t.Fatalf("%s request envelope rejection mismatch: got %+v want %+v", testCase["label"], importErr, expected)
		}
	}
}
...
func TestTemplateDirectorySessionInspectionTransportRejectionFixture(t *testing.T) {
	fixturePath := filepath.Join(repoRoot(t), "fixtures", "diagnostics", "slice-411-template-directory-session-inspection-transport-rejection", "template-directory-session-inspection-envelope-rejection.json")
	fixture := readJSONFixture(t, fixturePath)
	fixtureRoot := filepath.Dir(fixturePath)

	for _, rawCase := range fixture["cases"].([]any) {
		testCase := rawCase.(map[string]any)
		envelope := decodeSessionInspectionEnvelopeFromFixture(t, testCase["envelope"], fixtureRoot)
		expected := decodeSessionInspectionTransportImportErrorFromFixture(testCase["expected_error"])

		imported, importErr := asttemplate.ImportSessionInspectionEnvelope(envelope)
		if imported != nil {
			t.Fatalf("%s inspection rejection unexpectedly imported %+v", testCase["label"], imported)
		}
		if !reflect.DeepEqual(importErr, expected) {
			t.Fatalf("%s inspection rejection mismatch: got %+v want %+v", testCase["label"], importErr, expected)
		}
	}
}

func TestTemplateDirectorySessionInspectionEnvelopeApplicationFixture(t *testing.T) {
	fixturePath := filepath.Join(repoRoot(t), "fixtures", "diagnostics", "slice-412-template-directory-session-inspection-envelope-application", "template-directory-session-inspection-envelope-application.json")
	fixture := readJSONFixture(t, fixturePath)
	fixtureRoot := filepath.Dir(fixturePath)
	...
	for _, rawCase := range fixture["rejections"].([]any) {
		...
		imported, importErr := asttemplate.ImportSessionInspectionEnvelope(envelope)
		if imported != nil {
			t.Fatalf("%s inspection envelope rejection unexpectedly imported %+v", testCase["label"], imported)
		}
		if !reflect.DeepEqual(importErr, expected) {
			t.Fatalf("%s inspection envelope rejection mismatch: got %+v want %+v", testCase["label"], importErr, expected)
		}
	}
}
...
func decodeSessionRequestTransportImportErrorFromFixture(raw any) *asttemplate.SessionRequestTransportImportError {
	section := raw.(map[string]any)
	return &asttemplate.SessionRequestTransportImportError{
		Category: asttemplate.SessionRequestTransportImportErrorCategory(stringOrZero(section["category"])),
		Message:  stringOrZero(section["message"]),
	}
}

func decodeSessionOutcomeTransportImportErrorFromFixture(raw any) *asttemplate.SessionOutcomeTransportImportError {
	section := raw.(map[string]any)
	return &asttemplate.SessionOutcomeTransportImportError{
		Category: asttemplate.SessionOutcomeTransportImportErrorCategory(stringOrZero(section["category"])),
		Message:  stringOrZero(section["message"]),
	}
}

func decodeSessionInspectionTransportImportErrorFromFixture(raw any) *asttemplate.SessionInspectionTransportImportError {
	section := raw.(map[string]any)
